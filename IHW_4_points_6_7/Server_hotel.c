...
int main() {
    ...
    while (1) {
        ...
        } else if (strncmp(buffer, "get-occupancy", 13) == 0) {
            // Отправка информации о количестве свободных мест клиенту
            intToStr(MAX_ROOMS - occupied_rooms, buffer);
            if (sendto(sockfd, buffer, sizeof(buffer), 0, (struct sockaddr *)&client_addr, client_len) < 0) {
                perror("Ошибка при отправке информации о количестве мест клиенту");
                exit(1);
            }
        }
    }
